...
import uk.gov.hmrc.ngrnotify.model.ratepayer.{RatepayerPropertyLinksResponse, RegisterRatepayerRequest, RegisterRatepayerResponse}
...
class RatepayerController @Inject() (
   ...
   *  ------------
   *
   *    1. This controller derives from the BridgeAwareBackendController base class
   *       as that provides utilities for handling Bridge failure messages and converting them
   *       to propert HTTP responses.
   *
  ...
  // TODO Reimplement the following action methods as per above DESIGN NOTE 
  //     by replacing the old HipConnector with the new BridgeConnector
