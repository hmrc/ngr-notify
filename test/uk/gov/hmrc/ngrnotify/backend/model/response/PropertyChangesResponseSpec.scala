...
class PropertyChangesResponseSpec extends AnyFlatSpec with Matchers  {
  ...
  val propertyChangesResponseJson: JsValue = Json.parse(
    """
      |{
      |  "error": "random string"
      |}
  ...
  "RatepayerStatusResponse" should "serialize to JSON" in {
    Json.toJson(propertyChangesResponse) mustBe propertyChangesResponseJson
  }

  it should "deserialize from JSON" in {
    propertyChangesResponseJson.as[PropertyChangesResponse] mustBe propertyChangesResponse
  }
