...
import uk.gov.hmrc.ngrnotify.model.response.bridge.Compartments
import uk.gov.hmrc.ngrnotify.backend.testUtils.BridgeModelTestData.testCompartmentsPersonJob

class CompartmentsSpec extends AnyWordSpec with Matchers {

  private val compartmentsJson = Json.obj(
    "properties"    -> Json.arr(),
  ...
  "Compartments" should {
    ...
    "serialize to JSON correctly" in {
      val json = Json.toJson(testCompartmentsPersonJob)
      json shouldBe compartmentsJson
    }

    "deserialize from JSON correctly" in {
      val json         = compartmentsJson
      val compartments = json.as[Compartments]
      compartments shouldBe testCompartmentsPersonJob
    }

    "round-trip JSON serialization and deserialization" in {
      val original = compartmentsJson
      val json     = Json.toJson(original)
      val parsed   = json.as[Compartments]
      parsed shouldBe testCompartmentsPersonJob
    }
  }
}
