...
/**
  ...
  */
class EmailNotificationSpec extends AnyFlatSpec with Matchers with FakeObjects:

  "NotConnectedSubmission" should "be serialized/deserialized from JSON" in {
    val json = Json.toJson(prefilledEmailRegistrationSuccessful)
    json.as[EmailNotification] shouldBe prefilledEmailRegistrationSuccessful
  }
