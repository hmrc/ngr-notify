...
class StatusControllerSpec extends AnyWordAppSpec:
  ...
  "StatusController" should {
    ...
    ".getRatepayerStatus return 200" in {
      ...
      when(
        client.get(url(endingWith("/job/ratepayer/GGID123345/dashboard")))(using any[HeaderCarrier])
    ...
    ".getRatepayerStatus return 500 for wrong ID" in {
      val client = inject[HttpClientV2]
      when(
        client.get(url(endingWith("/job/ratepayer/1234567891255/dashboard")))(using any[HeaderCarrier])
    ...
    ".getRatepayerStatus return 500 if HIP response body is not a JSON" in {
      val client = inject[HttpClientV2]
      when(
        client.get(url(endingWith("/job/ratepayer/test_no_json/dashboard")))(using any[HeaderCarrier])
    ...
    ".getRatepayerStatus return 500 on HIP connection exception" in {
      val client = inject[HttpClientV2]
      when(
        client.get(url(endingWith("/job/ratepayer/test_hip_connection_error/dashboard")))(using any[HeaderCarrier])
